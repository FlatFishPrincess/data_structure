...
//		}
	...
	private int setHashIndex(int index) {
		System.out.println("hash code  " + index);
		System.out.println("with hashtable length: " + hashTable.length + "  index%length: " + index%hashTable.length);
		index = index % hashTable.length;
		System.out.println("hashtable index = " + index);
		if(index < 0)
			index = index + hashTable.length;
		return index;
	}
