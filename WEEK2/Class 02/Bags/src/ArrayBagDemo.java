...
public class ArrayBagDemo {
	public static void main(String[] args) {
		...
		// Adding strings
		System.out.println("Adding " + contentsOfBag.length + " strings to an initially empty bag with"
				+ " the capacity to hold more than " + contentsOfBag.length + " strings:");
	...
	private static void testFrequency(BagInterface<String> aBag, String[] tests) {
		...
		for (int index = 0; index < tests.length; index++) {
			...
			if (!aString.equals("") && (aString != null)) {
				System.out.println(
						"In this bag, the count of " + tests[index] + " is " + aBag.getFrequencyOf(tests[index]));
			} // end if
		} // end for
	} // end testFrequency
