...
//		String fileName = "FrequencyCounterData.txt"; 
		...
		try {
			while ((nextChar = reader.read()) != -1) {
				...
				if(characterFrequencies.containsKey(c)){ // if character is in the word table
					...
					CharNode newNode = new CharNode(c, 1); // Integer.valueof(1) => instead insert 1
	...
	static PriorityQueueInterface<CharNode> toPQ(HashMap<Character, CharNode> table) {
		...
		// -------In Class ---------- 
		// loop keyIterator then add queue to key 
	...
	static CharNode toTree(PriorityQueueInterface<CharNode> pq) {
		...
		while(pq.getSize() >1){
			...
			CharNode parentEntry  = new CharNode(null, entry1.getFrequency() + entry2.getFrequency());
			parentEntry.setLeftChild(entry1);
			parentEntry.setRightChild(entry2);
		}
		return null;
	}
