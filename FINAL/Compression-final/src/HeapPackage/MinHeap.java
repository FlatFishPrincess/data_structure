...
//		return root;
	...
	private void reheap(int rootIndex) {
		...
		while(!done && leftChildIndex <= lastIndex){
			...
			int smallerChildInex = leftChildIndex;
			if(heap[smallerChildInex].compareTo(heap[rightChildIndex]) > 0){
				// right is smaller
				smallerChildInex = rightChildIndex;
			}
//			compare orphan 
			if(orphan.compareTo(heap[smallerChildInex]) > 0){
				heap[rootIndex] = heap[smallerChildInex];
				rootIndex = smallerChildInex;
