 ...
 * 
 * Implement using ArrayListUsingListInterface.
 * 
 * @author Jiweon Park
 *
 */
public class CoinTossUsingAList {

	public static void main(String[] args) {

		// replace the below with an AList or an LList

		ListInterface<Coin> coins = new AList2<>();
		ListInterface<Coin> headsCoins = new AList2<>();
	...
	public static void moveCoins(ListInterface<Coin> coins, ListInterface<Coin> headsCoins) {
		...
		for(int i = 1; i<= coins.getLength(); i++){
			Coin coin = coins.getEntry(i);
			if(coin.isHeads()){
				coins.remove(i);
				i--;
				headsCoins.add(coin);
//				headsCoins.add(coins.remove(i));
	...
	public static void showCoins(ListInterface<Coin> coins, String header) {
		...
		for (int i = 1; i <= coins.getLength(); i++) {
			System.out.println(coins.getEntry(i));
		}
		System.out.println();
	}
	...
	public static int getTotalValue(ListInterface<Coin> coins) {
		int totalValue = 0;
		for(int i = 1; i <= coins.getLength(); i++)
			totalValue += coins.getEntry(i).getValue();
		return totalValue;
	}
