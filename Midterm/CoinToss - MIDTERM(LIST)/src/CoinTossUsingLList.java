 ...
 *
 */
public class CoinTossUsingLList {

	public static void main(String[] args) {

		// replace the below with an AList or an LList
		System.out.println("ConinTossUsingLList");
		ListInterface<Coin> coins = new LList2<>();
		ListInterface<Coin> headsCoins = new LList2<>();
	...
	public static void moveCoins(ListInterface<Coin> coins, ListInterface<Coin> headsCoins) {
		...
		for(int i = 1; i<= coins.getLength(); i++){
			if(coins.getEntry(i).isHeads()){
				Coin removedCoin = coins.remove(i);
				headsCoins.add(removedCoin);
				i--;
			} 
		}
	}
